...
function normalizeTransaction(t, account) {
  ...
  // Internal money movements (CC payments, account-to-account transfers,
  // broker activity, salary deposits, interest) are stored — but tagged
  // `Transfers & Payments` so they don't pollute spend totals while staying
  // visible in the Transfers view. Sign convention is preserved: positive =
  // money flowing out of this account, negative = money flowing in.
  const transferCategory = isNonSpendMovement(t, account, signed) ? "Transfers & Payments" : null;
...
function isNonSpendMovement(t, account, signedAmount) {
  ...
  // Plain "transfer" type with no clear purchase context is suspect; treat
  // bank-to-bank transfers as non-spend.
  if (/^(transfer|wire)$/i.test(t.type || "")) {
    return true;
  }
