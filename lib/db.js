...
// Idempotent: any account row missing a `status` field is treated as already
// activated. New accounts created via the Connect flow start with status
// "pending" and are filtered out of dashboards/sync until the user reviews
// them and assigns a name + type.
function migrateAccountStatus(data) {
  let touched = 0;
  for (const a of data.accounts) {
    if (!a.status) { a.status = "active"; touched++; }
  }
  if (touched) {
    saveDB(data);
    console.log(`  Schema migration: marked ${touched} existing account row${touched === 1 ? "" : "s"} as 'active'`);
  }
}
